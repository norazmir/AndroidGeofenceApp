...
public class StaffRegisterActivity extends AppCompatActivity implements View.OnClickListener {
    ...
    private void createStaff() {
        ...
        Staff  staff= new Staff(userId,userName, userAddress,userEmail, userPhoneNum);
        databaseUser.child("Staff").child(userId).setValue(staff).addOnCompleteListener(new OnCompleteListener<Void>() {
            @Override
            public void onComplete(@NonNull Task<Void> task) {
                if(task.isSuccessful()){
                    Toast.makeText(StaffRegisterActivity.this, "Register Success", Toast.LENGTH_SHORT).show();
                }else{
                    Toast.makeText(StaffRegisterActivity.this, task.getException().getMessage(), Toast.LENGTH_SHORT).show();
                }
            }
        });
    }
        ...
        mAuth.createUserWithEmailAndPassword(email,password).addOnCompleteListener(new OnCompleteListener<AuthResult>() {
            ...
            public void onComplete(@NonNull Task<AuthResult> task) {
                if(task.isSuccessful()){
                    createStaff();
                    finish();
                    startActivity(new Intent(StaffRegisterActivity.this, StaffMainActivity.class));
                }else{
                    Toast.makeText(StaffRegisterActivity.this,  task.getException().getMessage(), Toast.LENGTH_SHORT).show();
                }
            }
